...
def run_structural_audit():
    ...
    df_generated = build_base_features(train)
    ...
    missing_in_runtime = schema_set - runtime_set
    extra_in_runtime = runtime_set - schema_set
    ...
    if missing_in_runtime:
        ...
        logger.info("✓ All schema features present in runtime.")
    ...
    full_base = set(BASE_COLS)
    
    print("\n[STRUCTURAL_AUDIT_SUMMARY]")
    print(f"schema_match: {'PASS' if not missing_in_runtime and not extra_in_runtime else 'FAIL'}")
    print(f"feature_count_expected: {len(schema_features)}")
    print(f"feature_count_actual: {len(runtime_features)}")
    print(f"pca_decoupling_status: {'DECOUPLED (SAFE)' if len(runtime_features) > 400 else 'COUPLED (BROKEN)'}")
