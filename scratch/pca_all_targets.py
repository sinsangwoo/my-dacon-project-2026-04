...
targets = ['order_inflow_15m', 'robot_utilization', 'congestion_score', 'near_collision_15m']
...
while True:
    ...
    for t in targets:
        if t in current:
            idx = current.index(t)
            load_scores[idx] = 999.0 # Don't remove targets
    
    worst_idx = np.argmin(load_scores)
    current.pop(worst_idx)
    
    if len(current) <= len(targets):
        print("FAILED to reach 0.8 with targets.")
        break
