...
def attack_zone_transformation():
    print("--- [MISSION: CLUSTER 1 -> TRUE ATTACK ZONE] ---")
    ...
    # Identify Cluster 1 (from previous mission's clustering)
    # Using low-confidence p <= 0.4
    low_p_mask = p_raw <= 0.4
    X_low = X_val[low_p_mask]
    kmeans_initial = KMeans(n_clusters=3, random_state=42).fit(X_low)
    # Identify Cluster 1 based on previous Avg_Gain logic (Cluster 1 was the 요충지)
    cluster_initial = kmeans_initial.labels_
    c1_mask_in_low = cluster_initial == 1
    c1_indices = np.where(low_p_mask)[0][c1_mask_in_low]
    
    X_c1 = X_val[c1_indices]
    y_gt_c1 = y_gt_val[c1_indices]
    
    # [TASK 1: High-resolution Decomposition of Cluster 1]
    ...
    for sc in range(2):
        ...
        actual_gain = np.abs(y_gt_c1[sc_mask] - y_base[c1_indices][sc_mask]) - np.abs(y_gt_c1[sc_mask] - y_tail[c1_indices][sc_mask])
        sub_results.append({"Sub-Cluster": sc, "Size": np.sum(sc_mask), "MAE": mean_absolute_error(y_gt_c1[sc_mask], y_tail[c1_indices][sc_mask]), "Avg_Gain": np.mean(actual_gain), "Noise_Ratio": 1 - np.mean(y_val[c1_indices][sc_mask])})
    print(pd.DataFrame(sub_results))

    # [TASK 2: Noise Kill Condition]
    print("\n[TASK 2: NOISE KILL CONDITION]")
    pred_var_c1 = np.var([y_t1[c1_indices], y_t2[c1_indices]], axis=0)
    # Kill if Variance > P90 of Cluster 1
    kill_mask_var = pred_var_c1 > np.percentile(pred_var_c1, 90)
    
    # [TASK 3: Expected Value Filter]
    print("\n[TASK 3: EXPECTED VALUE FILTER]")
    # EV(i) = p(i) * Gain(i) - (1-p(i)) * Cost(i)
    # Sample-wise calculation using actuals for simulation
    ...
    final_comp = [
        ...
        {"Method": "Cluster 1 (Full)", "MAE": mean_absolute_error(y_gt_val, np.where(low_p_mask & (cluster_initial == 1), y_tail, y_base))},
        {"Method": "Final Attack Zone", "MAE": mean_absolute_error(y_gt_val, y_final)}
    ]
    print(pd.DataFrame(final_comp))

if __name__ == "__main__":
    attack_zone_transformation()
